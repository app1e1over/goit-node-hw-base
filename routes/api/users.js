...
router.get("/verify/:verificationToken", async (req, res, next)=>{
  const us = await users.findByVerification(req.params.verificationToken);
  if(us){    
    res.status(200);
...
router.post("/verify", async (req, res, next)=>{
  const us = await users.sendMail(req.body.email);
  if(us){    
    res.status(200);
    res.json({message: "Sent successfuly"});
  }else{
    res.status(404);
    res.json({message: "User not found or email is already verified"});
  }
})
